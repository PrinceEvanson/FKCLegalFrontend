  ...
  return (
    <>
      <nav className="bg-white dark:bg-fkcBlack text-gray-900 dark:text-white border-b border-gray-200 dark:border-fkcGold/30 sticky top-0 z-50 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            <Link to="/" className="flex items-center gap-3">
              <img src={fkcLogo} alt="FKC Legal Logo" className="h-16 w-auto object-contain" />
            </Link>
            ...
            <div className="hidden md:flex items-center space-x-6 text-xs font-medium">
              ...
              <div className="relative" onMouseEnter={() => setDropdownOpen(true)} onMouseLeave={() => setDropdownOpen(false)}>
                ...
                {dropdownOpen && (
                  <div className="absolute top-full left-0 w-80 bg-white dark:bg-fkcBlack border border-gray-200 dark:border-fkcGold/40 shadow-2xl py-2 rounded-md max-h-[440px] overflow-y-auto z-50">
                    <Link
                      to="/services"
                      className="block px-4 py-2 text-xs font-bold uppercase tracking-wider text-fkcDarkGold dark:text-fkcGold hover:bg-gray-100 dark:hover:bg-fkcBlue/30 border-b border-gray-200 dark:border-fkcGold/20 mb-1"
                    >
                      All Practice Areas
                    </Link>
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                ...
                className="md:hidden p-2 rounded-xl border border-gray-300 dark:border-fkcGold/40 text-gray-900 dark:text-white hover:text-fkcGold transition cursor-pointer"
        ...
        {isOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-white dark:bg-fkcBlack border-b border-gray-200 dark:border-fkcGold/30 shadow-2xl py-6 px-6 space-y-4 max-h-[80vh] overflow-y-auto">
            <div className="flex flex-col space-y-3 text-sm font-medium">
              <Link to="/who-we-help" className="py-2 hover:text-fkcGold transition">WHO WE HELP</Link>

              <div>
                <button
                  onClick={() => setMobileServicesOpen(!mobileServicesOpen)}
                  className="flex items-center justify-between w-full py-2 hover:text-fkcGold transition"
                >
                  <span>SERVICES</span>
                ...
                {mobileServicesOpen && (
                  <div className="pl-4 py-2 space-y-2 border-l-2 border-fkcGold/30 my-2">
                    <Link to="/services" className="block py-1.5 text-xs font-bold uppercase tracking-wider text-fkcGold">
                      All Practice Areas
                    </Link>
                    {servicesList.map((service, idx) => (
                      <Link key={idx} to={service.path} className="block py-1.5 text-xs text-gray-700 dark:text-gray-300 hover:text-fkcGold transition">
                        {service.name}
                      </Link>
                    ))}
                  </div>
              ...
              <Link to="/knowledge-lab" className="py-2 hover:text-fkcGold transition">KNOWLEDGE LAB</Link>
              <Link to="/diplomat" className="py-2 hover:text-fkcGold transition">DIPLOMAT & EX-DIPLOMAT</Link>
              <Link to="/about-us" className="py-2 hover:text-fkcGold transition">ABOUT</Link>
              <Link to="/career" className="py-2 hover:text-fkcGold transition">CAREER</Link>
              <Link to="/contact" className="py-2 hover:text-fkcGold transition">CONTACT US</Link>
            </div>
          </div>
        )}
      </nav>
